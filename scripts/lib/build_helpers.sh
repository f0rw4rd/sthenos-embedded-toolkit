...
verify_sha512() {
    ...
    if [ -z "$expected_sha512" ]; then
        log_error "SHA512 checksum required but missing for $description"
        return 1
    fi
...
check_cached_file() {
    ...
    if [ -z "$expected_sha512" ]; then
        log "SHA512 was not set! for $file_path"
        return 1
    fi    
...
check_binary_exists() {
    ...
    if [ -s "$binary_path" ] && [ "${SKIP_IF_EXISTS:-true}" = "true" ]; then
        local size=$(ls -lh "$binary_path" | awk '{print $5}')
        log "[$arch] Already built: $binary_path ($size)"
        return 0
    fi
    [ -f "$binary_path" ] && [ ! -s "$binary_path" ] && rm -f "$binary_path"
    return 1
}
...
install_binary() {
    ...
    if [ ! -s "$source_file" ]; then
        log_tool_error "$tool_name" "Binary not found or empty: $source_file"
        return 1
    fi

    # Try to strip the binary, but don't fail if it doesn't work (e.g., Windows PE)
    if ! $STRIP "$source_file" 2>/dev/null; then
        log_tool_warn "$tool_name" "Could not strip binary for $arch (may be cross-platform binary)"
    fi

    if [ ! -s "$source_file" ]; then
        log_tool_error "$tool_name" "Strip produced empty binary for $arch: $source_file"
        return 1
    fi
...
ac_cv_c_bigendian=$(if is_big_endian_arch "$arch"; then echo "yes"; else echo "no"; fi)
ac_cv_c_littleendian=$(if is_big_endian_arch "$arch"; then echo "no"; else echo "yes"; fi)
ac_cv_func_setpgrp_void=yes
ac_cv_func_setgrent_void=yes
ac_cv_func_getpgrp_void=yes
ac_cv_func_getgrent_void=yes
ac_cv_sizeof_int=4
ac_cv_sizeof_long=$(get_pointer_size "$arch")
ac_cv_sizeof_long_long=8
ac_cv_sizeof_void_p=$(get_pointer_size "$arch")
ac_cv_sizeof_size_t=$(get_pointer_size "$arch")
...
generate_socat_cross_cache() {
    local arch=$1
    local cache_file=$2
    local ptr_size=$(get_pointer_size "$arch")
    local bigendian="no"
    if is_big_endian_arch "$arch"; then
        bigendian="yes"
    fi
...
ac_cv_sizeof_int=4
ac_cv_sizeof_long=${ptr_size}
ac_cv_sizeof_long_long=8
ac_cv_sizeof_off_t=8
ac_cv_sizeof_off64_t=8
ac_cv_sizeof_size_t=${ptr_size}
ac_cv_sizeof_time_t=${ptr_size}
ac_cv_sizeof_void_p=${ptr_size}
...
get_libc_suffix() {
    # Zig targets use their bundled libc (Darwin libSystem, BSD libc, etc.),
    # not musl or glibc. Force "zig" suffix regardless of LIBC_TYPE to avoid
    # mislabelling outputs as .musl/.glibc on non-Linux targets.
    if [ "${USE_ZIG:-0}" = "1" ]; then
        echo "zig"
        return
    fi
...
get_output_path() {
    local arch=$1
    local tool_name=$2
    # Windows PE binaries need the .exe extension so the OS recognises them
    local ext=""
    if [[ "$arch" == *_windows ]]; then
        ext=".exe"
    fi
    echo "/build/output/$arch/${tool_name}.$(get_libc_suffix)${ext}"
}
...
get_output_dir() {
    ...
    echo "/build/output/$arch/${tool_name}.$(get_libc_suffix)"
}
...
update_config_scripts() {
    ...
    for f in config.sub config.guess; do
        if [ -f "/usr/share/misc/$f" ]; then
            find "$source_dir" -name "$f" -exec cp "/usr/share/misc/$f" {} \;
        fi
    done
}

export -f update_config_scripts

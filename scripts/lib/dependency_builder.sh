...
build_dependency_generic() {
    ...
    # Use DEPS_PREFIX to separate cache by compiler type (gcc/zig)
    local prefix="${DEPS_PREFIX:-gcc}"
    local cache_dir="$DEPS_CACHE_DIR/$prefix/$arch/$dep_name-$version"
    ...
    # Zig sets CC="zig cc -target ..." in setup_arch; export_cross_compiler
    # would overwrite it with "gcc" (empty CROSS_COMPILE prefix) and break
    # cross-compilation for macOS/BSD/Windows targets.
    if [ "$LIBC_TYPE" != "glibc" ] && [ "${USE_ZIG:-0}" != "1" ]; then
...
configure_openssl() {
    ...
    # Restore CROSS_COMPILE so downstream builds work
...
configure_libpcap() {
    ...
    # Pick the right pcap capture backend for the target OS
    local pcap_backend="linux"
    case "$arch" in
        *_macos|*_freebsd|*_openbsd|*_netbsd|*_dragonfly) pcap_backend="bpf" ;;
    esac

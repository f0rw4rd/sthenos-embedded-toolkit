...
SUPPORTED_OS="linux,android,freebsd,openbsd,netbsd,macos"  # Windows needs winsock2 port (upstream uses sys/socket.h)
...
build_microsocks() {
    ...
    make -j$(nproc) CFLAGS="$cflags" LDFLAGS="$ldflags -lpthread" || {
        log_tool_error "microsocks" "Build failed for $arch"
        cleanup_build_dir "$build_dir"
        return 1
    }
    
    $STRIP microsocks 2>/dev/null || true
